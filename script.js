...
// ✅ Wechsel auf schnellen, freien Solana RPC
const connection = new Connection('https://rpc.triton.one');
  ...
  try {
    ...
    for (const nft of nftAccounts) {
      ...
      try {
        ...
        if (!metadataUri) {
          console.warn(`[DEBUG] No metadata URI for mint ${mintAddress}`);
          continue;
        }
        ...
        console.error(`[DEBUG] Error processing NFT ${mintAddress}:`, e);
        continue;
      }
    }
