...
async function forceUnstakeSingleNFT(mintAddress) {
  ...
  instructions.push(new TransactionInstruction({
    ...
    keys: [
      ...
      { pubkey: VAULT_ADDRESS, isSigner: true, isWritable: false },
    ],
    data: new Uint8Array([3, 0, 0, 0, 1, 0, 0, 0]) // Transfer 1 Token (correct binary format)
  }));
...
function createAssociatedTokenAccountInstruction(payer, ata, owner, mint) {
  ...
  return new TransactionInstruction({
    ...
    keys,
    data: new Uint8Array([]), // 🛠 FIX: Browser-kompatibel
  });
}
