  ...
  try {
    ...
    for (const nft of nftAccounts) {
      ...
      try {
        ...
        if (!metadataUri) {
          console.warn(`No metadata URI for mint ${mintAddress}`);
          continue; // NFT überspringen
        }
        ...
        console.error(`Error processing NFT ${mintAddress}:`, e);
        continue; // Wenn Fehler, nächsten NFT laden
      }
    }
